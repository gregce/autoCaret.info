...
examples <- function(caption = TRUE, team = FALSE, other = FALSE) {
  
  # read examples into data frame (so we can easily sort/filter/etc)
  examples <- yaml::yaml.load_file("thumbnails.yml")
  examples <- plyr::ldply(examples, data.frame, stringsAsFactors=FALSE)
  
  # filter if requested
  if (team)
    examples <- subset(examples, examples$team == TRUE)
  if (other)
    examples <- subset(examples, examples$other == TRUE)
